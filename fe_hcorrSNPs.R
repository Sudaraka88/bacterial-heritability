...
# Core Only
# mxCC = readRDS("Pensar2019_pen+cef/mapped_mx_NC_MIC.rds") # load SNPs
# pheno = readRDS("pen.mic_cef.mic_pheno_reduced.rds")
# mxCC = readRDS("../Genotype2/Pensar2019/mapped_mx_NC_MIC.rds") # load SNPs
# pheno = readRDS("Results/FBCLUSTER_GLS/mapped_pheno_Wclust_MIC.rds")

# Core + Accessory
mxCC = readRDS("../Accessory_Genome/DATASET2/Pensar2019/acc_mic_core_mx_NC.rds") # load SNPs
...
plt.mic = ggplot(df.plot_lmm) + geom_point(aes(x = pos, y = pval, col = ret), alpha = 0.5) + 
  ...
  xlab("SNP position") + ylab("-10log(pval): LMM") + ylim(ymin, ymax) + 
