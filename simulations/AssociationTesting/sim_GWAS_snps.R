...
gwas_pipeline = function(ph, formi, pheno_data, sim_mx, mxCC){
  ...
  y_t <- crossprod(W, y)
  C_t <- crossprod(W, C)
  # C_t = unique.matrix(C_t_d, MARGIN = 2)
...
mxCC = readRDS("../Accessory_Genome/DATASET2/snpDat/cd_core_uqepi_mx.rds")
sim_mx = read.table("../pySEER/DATASET3/phylosim_cdacute.tsv") # These row names/col names need to be modified
pheno = read.table(ph_[1], header = T) # This is the full phenotype
pheno$samples = sapply(pheno$samples, rename_simmx)
sim_mx = sim_mx_cleaner(sim_mx, pheno$samples)
...
for(ph in ph_){
  ...
  # if(ph == "y"){
  print(paste("Reading phenotype:", ph))
  pheno = read.table(ph, header = T) # This is the full phenotype
  
  pheno$samples = sapply(pheno$samples, rename_simmx)
  
  checkAlignment(pheno$samples, rownames(mxCC))
  
 
  pheno_data = data.frame(ids = pheno$samples, y = pheno$y, dummy = rep(1,nrow(pheno)))
