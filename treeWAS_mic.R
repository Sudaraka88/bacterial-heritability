...
if(length(args) == 0) {
  ...
  return(0)
} else if(!args[1] %in% c("cef.mic","pen.mic")) {
  print("Input must be either phenotype <cef.mic> OR <pen.mic>")
  return(0)
} else ph = args[1] # phenotype
...
# ph = c("acute", "cd") # phenotype(s) to process
print("Loading files...")

############ Deal with the genotype first ###########
# snp_pos = readRDS("pos_mic.rds") # not provided
# genotype = readRDS("mapped_mx_CC_MIC.rds") # not provided
...
# saveRDS(snps.unique, "snps.unique_mic.rds") # unique snps

snps.unique = readRDS("snps.unique_mic.rds") # not provided
print("Loaded SNPs: snps.unique_mic.rds")
pheno = readRDS("pheno_mic") # not provided
print("Loaded pheno: mapped_pheno_MIC.rds")
# With ClonalFrameML
maela_tree = read.tree("Trees/cfrm_MIC.labelled_tree.newick") # clonalFrame Corrected 

##############################################################################################
# reset tree labels to align with other isolate names (# instead of _)
maela_tree$tip.label = unname(sapply(maela_tree$tip.label, function(x) refmt(x)))
# Drop the additional sequences from the phenotype 20210124
pheno = pheno[which(pheno$sampleID %in% maela_tree$tip.label), ]
# and Genotype
snps.unique = snps.unique[which(rownames(snps.unique) %in% maela_tree$tip.label),] 
str(snps.unique)
# check if all sample IDs match with maela tree tip labels
print(paste("Tip.label and pheno$sample ID match?", all(pheno$sampleID %in% maela_tree$tip.label)))
print(paste("Tip.label and snps.unique$rownames match?", all(rownames(snps.unique) %in% maela_tree$tip.label)))
...
if(ph == "pen.mic") phenotype = log10(pheno$Penicillin.MIC) # Load CD pheno
if(ph == "cef.mic") phenotype = log10(pheno$Ceftriaxone.MIC)  # Load acute pheno

names(phenotype) = pheno$sampleID
print("Loaded Phenotype")
str(phenotype)

print(paste("Final data size: nSEQs x nSNPs:", nSEQs, "x", nSNPs))

#####################################################################
##################### RECONSTRUCTION ################################
st = nSEQs + 1
en = length(maela_tree$edge.length) + 1
if(!file.exists("snps.rec_stpd_MIC.rds")){
  t = Sys.time()
  ...
  saveRDS(snps.rec_stpd, "snps.rec_stpd_MIC.rds")
  print("snps recon complete...")
} else {
  snps.rec_stpd = readRDS("snps.rec_stpd_MIC.rds")
  print("Loaded SNP RECON: snps.rec_stpd_MIC.rds")
  # str(snps.rec_stpd)
}
####################### SIMULATION ########################
if(!file.exists("simsnps.unique_MIC.rds")){
  print("Begin snp sim")
  t = Sys.time()
  ...
  print(paste("Done, ET:", Sys.time() - t))
  print("Saving all simulated SNPs: simsnps_MIC.rds")
  saveRDS(simsnps, "simsnps_MIC.rds")
  ...
  saveRDS(simsnps.unique, "simsnps.unique_MIC.rds")
  print(str(simsnps.unique))
} else {
  simsnps.unique = readRDS("simsnps.unique_MIC.rds")
  print("Loaded SIM SNPS: simsnps.unique_MIC.rds")
  # str(simsnps.unique)
}

##################### RECONSTRUCTION ################################
# This is very time and memory consuming, avoid repeating at all costs!
if(!file.exists("simsnps.rec_stpd_MIC.rds")){
  ...
  t = Sys.time()
  simsnps.rec_stpd = foreach(i = 1:nSIMSNPs, .combine = "cbind", .verbose = F) %dopar% {
  ...
  saveRDS(simsnps.rec_stpd, "simsnps.rec_stpd_MIC.rds")
  print("simulated SNPs recon complete...")
} else {
  simsnps.rec_stpd = readRDS("simsnps.rec_stpd_MIC.rds")
  print("Loaded RECON SIM SNPS: simsnps.rec_stpd_MIC.rds")
}
...
if(!file.exists(paste("phen.rec_stpd_MIC_", ph, ".rds", sep = ""))){
  print("Begin phenotype recon")
  # Let's make the ancestral reconstructions
  phen.rec = asr(var = phenotype, tree = maela_tree, type = "ML", method = "continuous")
  phen.rec_strpd = phen.rec[st:en]
  saveRDS(phen.rec_strpd, paste("phen.rec_stpd_MIC_", ph, ".rds", sep = ""))
  print("Complete...")
} else {
  phen.rec_strpd = readRDS(paste("phen.rec_stpd_MIC_", ph, ".rds", sep = ""))
  print(paste("Loaded phen.rec_stpd_MIC_", ph, ".rds", sep = ""))
}
...
for(i in 1:length(test)){
  ...
  saveRDS(assoc.scores, paste(ph, "_t_", test[i], "_sc_MIC.rds", sep = ""))
  ...
  sig.list[[i]] = get.sig.snps(corr.dat = assoc.scores$corr.dat,
                               ...
                               p.value.by = "count")
  saveRDS(sig.list[[i]], paste(ph, "_t_", test[i], "_sigsnps_MIC.rds", sep = ""))
  ######################################################################
  ############################# Plot Results ###########################
  ######################################################################
  ...
  ## plot:
  png(file = paste("p_", ph, "_", test[i], "mhp_MIC.png", sep = ""), width = 1920, height = 1080, type = "cairo")
  ...
  ####################################
  png(file = paste("p_", ph, "_", test[i], "null_dist_MIC.png", sep = ""), width = 1920, height = 1080, type = "cairo")
  if(test[i] == "fisher"){
  ...
  dev.off()
  png(file = paste("p_", ph, "_", test[i], "emp_dist_MIC.png", sep = ""), width = 1920, height = 1080, type = "cairo")
  if(test[i] == "fisher"){
    ## Generate one histogram per test:
    plot_sig_snps(corr.dat = -log10(abs(sig.list[[i]]$corr.dat)),
                  corr.sim = -log10(abs(sig.list[[i]]$corr.sim)),
...
names(sig.list) = test
saveRDS(sig.list, paste("sig.list_MIC_", ph, ".rds", sep = ""))
