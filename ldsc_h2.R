...
perform_ldsc = function(ph, acc = FALSE, fec = FALSE){
  ...
  if(ph=="cd"){
    if(acc){
      r2 = readRDS("r2s_cd_acc.rds") # These are the r2 outputs
      core_snps = readRDS("cd_core_mx_NC.rds") # core_genome
      acc_genes = readRDS("cd_acc.rds")
      acc_genes = acc_gene_cleaner(acc_genes)
      mxCC = cbind(core_snps, acc_genes)
    } else{
      r2 = readRDS("r2s_cd.rds") # These are the r2 outputs
      mxCC = readRDS("cd_core_mx_NC.rds")
    }
    r2scores = r2$r2scores
    r2w = r2$r2weights
    pheno = readRDS("pheno_cd.rds") # load pheno
    # pos_original = readRDS("../Accessory_Genome/DATASET2/Pensar2019/acc_cd.acute_core_pos_NC.rds") #  load all pos in mxCC
    passoc = readRDS("cd_passoc_gls_nocov.rds") # This is the GLS output, only to find retained pos
    if(!acc){
    ...
    if(acc){
      r2 = readRDS("r2s_mic_acc.rds") # These are the r2 scores
      core_snps = readRDS("acc_mic_core_mx_NC.rds") # core_genome
      acc_genes = readRDS("pen.mic_cef.mic_accessory_genome.rds")
      acc_genes = acc_gene_cleaner(acc_genes)
      mxCC = cbind(core_snps, acc_genes)
    } else {
      r2 = readRDS("r2s_mic.rds") # These are the r2 scores
      mxCC = readRDS("acc_mic_core_mx_NC.rds")
    }
    r2scores = r2$r2scores
    r2w = r2$r2weights
    pheno = readRDS("pheno_mic.rds") # load pheno
    # pos_original = readRDS("../Accessory_Genome/DATASET2/Pensar2019/acc_mic_core_pos_NC.rds")
    passoc = readRDS("pen.mic_passoc_gls_nocov.rds")
    if(!acc){
      # acc_genes = grep("group", passoc$tab$predictor) # idxes of acc_genes
      acc_genes = which(is.na(as.numeric(passoc$tab$predictor))) # Accessory gene names cannot be converted to numeric, catch them using NAs during coercion
      pos_retained = as.numeric(passoc$tab$predictor[-acc_genes]) # Retained SNPs
