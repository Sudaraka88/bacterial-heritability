...
mkplt_gwas = function(passoc_gls, lab_pos = 0, pheno_type, sim_mx_type, shape_snps = NULL, offset = 2350000, test = "permi"){
  ...
  if(test == "bf"){
    test_lines = data.frame(intercepts = c(-log10(0.05/length(passoc_gls$tab$predictor)),
  ...
  gw_sig_idx = which(df.plot_lmm$pval[coreacc] > test_lines$intercepts[1])
  ...
  ymax = max(c(lgpvs, test_lines$intercepts))
  if(is.null(shape_snps)){
  ...
  plt_mic = plt_mic + geom_abline(data = test_lines, aes(intercept = intercepts, slope = slopes, linetype = Sig))
  if(!is.null(lab_pos)){
...
gwas_pipeline = function(ph, formi, pheno_data, sim_mx, mxCC){
  ...
  if(file.exists(paste("OUT/mod_", ph, ".rds", sep = ""))){
    ...
    if(ph == "cd"){
      mod = lme4qtl::relmatLmer(y ~ (1|hostID) + (1|ids), pheno_data, relmat = list(ids = sim_mx), calc.derivs = FALSE)
    } else {
  ...
  y_t <- crossprod(W, y)
  C_t <- crossprod(W, C)
  C_t = unique.matrix(C_t, MARGIN = 2)
  ...
  doParallel::registerDoParallel(cores = 8L)
  if(!file.exists(paste("OUT/gapgwas_", ph, ".rds", sep = ""))){
    out = foreach(i = 1:ncol(mxCC), .combine='rbind') %dopar% {
      tbl = tableC(mxCC[,i])/n
      X = unname(crossprod(W, as.numeric(scale(mxCC[,i]))))
      mdlx = lm('y~.', data.frame(y = y_t, c = C_t, x = X))
      unname(c(summary(mdlx)$coefficients[scol,], tbl[which(names(tbl) == 1)], n))
    }
    out = data.frame(out)
    out = cbind(pos, out)
    colnames(out) = c("pos", "beta", "se","t","p","gf","samples")
    saveRDS(out, paste("OUT/gapgwas_", ph, ".rds", sep = ""))
  } else {
    out = readRDS(paste("OUT/gapgwas_", ph, ".rds", sep = ""))
  }
  ...
  plt_dat = data.frame(pos = pos, p = -log10(out$p), gap_freq = out$gf)
  ...
  ggsave(paste("PLOTS/gapgwas_", ph, ".png", sep = ""), plt, width = 12)
...
for(ph in ph_){
  ...
  if(ph == "cd"){
    ...
    pheno = readRDS("pheno_cd_uqepi_last.rds") # This is a cd with unique episodes only
    
    print("Reading gap_snps: cd_core_uqepi_gap.rds")
    
    mxCC = readRDS("cd_core_uqepi_gap.rds")
    mx_idxord = c()
    ...
    print("Reading gap_snps: ../Accessory_Genome/DATASET2/gapDat/acc_mic_core_gap.rds")
    mxCC = readRDS("../Accessory_Genome/DATASET2/gapDat/acc_mic_core_gap.rds") # gap dataset
    checkAlignment(pheno$sampleID, rownames(mxCC))
    ...
    sim_mx = read.table("../pySEER/DATASET3/phylosim_mic.tsv") # These row names/col names need to be modified
    sim_mx = sim_mx_cleaner(sim_mx, pheno$sampleID)
  }
